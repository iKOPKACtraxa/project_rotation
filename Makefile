...
migrate:
	...
	psql -p 5432 -h localhost -U postgres -c "CREATE DATABASE calendardb"
	goose -dir migrations postgres "user=someuser dbname=calendardb sslmode=disable" up
unmigrate: 
	goose -dir migrations postgres "user=someuser dbname=calendardb sslmode=disable" down
	psql -p 5432 -h localhost -U postgres -c "DROP DATABASE calendardb"
